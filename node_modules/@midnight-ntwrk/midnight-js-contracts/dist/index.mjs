import { ContractState as ContractState$1, sampleSigningKey } from '@midnight-ntwrk/compact-runtime';
import { Transaction, makeContractExecutableRuntime, asEffectOption, asContractAddress, exitResultOrError, SucceedEntirely } from '@midnight-ntwrk/midnight-js-types';
import 'fs';
import 'path';
import { ContractExecutable } from '@midnight-ntwrk/compact-js';
import { getNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import { parseCoinPublicKeyToHex, parseEncPublicKeyToHex, assertDefined, assertIsContractAddress, ttlOneHour, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { ProvableCircuitId, VerifierKey } from '@midnight-ntwrk/compact-js/effect/Contract';
import { ZswapTransient, ZswapInput, ZswapOffer, ZswapOutput, Intent, ContractCallPrototype, communicationCommitmentRandomness, UnshieldedOffer, ContractDeploy, ContractState } from '@midnight-ntwrk/ledger-v8';
...
const unprovenTxFromContractUpdates = async (updateAndSignFn) => {
    return Transaction.fromParts(getNetworkId(), undefined, undefined, Intent.new(ttlOneHour()).addMaintenanceUpdate(await updateAndSignFn()));
};
...
async function createUnprovenCallTxFromInitialStates(zkConfigProvider, options, walletEncryptionPublicKey) {
    ...
    assertIsContractAddress(contractAddress);
    assertDefined(ContractExecutable.make(options.compiledContract)
...
contractAddress, privateStateId) => {
    ...
    return ContractExecutable.make(compiledContract).getProvableCircuitIds().reduce((acc, circuitId) => ({
        ...acc,
        [circuitId]: (...args) => {
...
async function findDeployedContract(providers, options) {
    ...
    const verifierKeys = await providers.zkConfigProvider.getVerifierKeys(ContractExecutable.make(compiledContract).getProvableCircuitIds());
