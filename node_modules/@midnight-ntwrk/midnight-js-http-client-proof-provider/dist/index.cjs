...
const getKeyMaterial = async (zkConfigProvider, keyLocation) => {
    try {
        ...
        return midnightJsTypes.zkConfigToProvingKeyMaterial(zkConfig);
    }
    catch {
        return undefined;
    }
...
const httpClientProvingProvider = (url, zkConfigProvider, config) => {
    ...
    if (checkUrl.protocol !== 'http:' && checkUrl.protocol !== 'https:') {
        throw new midnightJsTypes.InvalidProtocolSchemeError(checkUrl.protocol, ['http:', 'https:']);
    }
    if (proveUrl.protocol !== 'http:' && proveUrl.protocol !== 'https:') {
        throw new midnightJsTypes.InvalidProtocolSchemeError(proveUrl.protocol, ['http:', 'https:']);
    }
    ...
    return {
        async check(serializedPreimage, keyLocation) {
            ...
            const payload = ledgerV8.createCheckPayload(serializedPreimage, keyMaterial?.ir);
            const result = await makeHttpRequest(checkUrl, payload, timeout, headers);
            return ledgerV8.parseCheckResult(result);
        },
        async prove(serializedPreimage, keyLocation, overwriteBindingInput) {
            const keyMaterial = await getKeyMaterial(zkConfigProvider, keyLocation);
            const payload = ledgerV8.createProvingPayload(serializedPreimage, overwriteBindingInput, keyMaterial);
...
exports.httpClientProvingProvider = httpClientProvingProvider;
