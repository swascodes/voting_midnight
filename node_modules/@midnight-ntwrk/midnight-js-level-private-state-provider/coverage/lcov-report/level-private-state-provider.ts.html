...
<head>
    <title>Code coverage report for level-private-state-provider.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="index.html">All files</a> level-private-state-provider.ts</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">91.15% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>433/475</span>
            </div>
        
            
            <div class='fl pad1y space-right2'>
                <span class="strong">79.33% </span>
                <span class="quiet">Branches</span>
                <span class='fraction'>192/242</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                <span class="strong">91.29% </span>
                <span class="quiet">Lines</span>
                <span class='fraction'>430/471</span>
            </div>
...
<a name='L1347'></a><a href='#L1347'>1347</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
&nbsp;
  ...
  <span class="missing-if-branch" title="if path not taken" >I</span>if (existingPromise) {
<span class="cstat-no" title="statement not covered" >    return existingPromise;</span>
  }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >      if (!(error &amp;&amp; typeof error === 'object' &amp;&amp; 'code' in error &amp;&amp; error.code === 'LEVEL_NOT_FOUND')) {</span>
...
&nbsp;
  while (passwordRotationLocks.has(lockKey)) {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (Date.now() - startWait &gt; timeoutMs) {
...
const isDecryptionError = (error: unknown): boolean =&gt; {
  <span class="missing-if-branch" title="if path not taken" >I</span>if (!(error instanceof Error)) <span class="cstat-no" title="statement not covered" >return false;</span>
  const message = error.message.toLowerCase();
  return (
    message.includes('unsupported state') ||
<span class="branch-1 cbranch-no" title="branch not covered" >    message.includes('salt mismatch') </span>||
<span class="branch-2 cbranch-no" title="branch not covered" >    message.includes('invalid encrypted data') </span>||
<span class="branch-3 cbranch-no" title="branch not covered" >    message.includes('bad decrypt') </span>||
<span class="branch-4 cbranch-no" title="branch not covered" >    message.includes('unable to authenticate')</span>
  );
};
&nbsp;
...
&nbsp;
        if (!firstEntryValidated) {
          try {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (isDecryptionError(error)) {
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (shouldProceed &amp;&amp; !hasMatchingData) {
<span class="cstat-no" title="statement not covered" >        return { entriesMigrated: 0 };</span>
      }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >        decryptedValue = encryptedValue;</span>
        const reEncrypted = <span class="cstat-no" title="statement not covered" >encryption.encrypt(encryptedValue);</span>
<span class="cstat-no" title="statement not covered" >        await subLevel.put(key, reEncrypted);</span>
      }
&nbsp;
...
&nbsp;
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (error &amp;&amp; typeof error === 'object' &amp;&amp; 'code' in error &amp;&amp; error.code === 'LEVEL_NOT_FOUND') {
<span class="cstat-no" title="statement not covered" >        return null;</span>
      }
      throw error;
    }
...
&nbsp;
      if (StorageEncryption.isEncrypted(encryptedValue)) {
        const version = StorageEncryption.getVersion(encryptedValue);
        <span class="missing-if-branch" title="if path not taken" >I</span>if (version === 1) {
<span class="cstat-no" title="statement not covered" >          if (password === null) {</span>
<span class="cstat-no" title="statement not covered" >            password = await getPasswordFromProvider(passwordProvider);</span>
          }
<span class="cstat-no" title="statement not covered" >          decryptedValue = encryption.decryptWithPassword(encryptedValue, password);</span>
<span class="cstat-no" title="statement not covered" >          needsReEncryption = true;</span>
        } else {
          decryptedValue = encryption.decrypt(encryptedValue);
        }
      } else <span class="missing-if-branch" title="else path not taken" >E</span>{
...
&nbsp;
      <span class="missing-if-branch" title="if path not taken" >I</span>if (needsReEncryption) {
        const reEncrypted = <span class="cstat-no" title="statement not covered" >encryption.encrypt(decryptedValue);</span>
<span class="cstat-no" title="statement not covered" >        await subLevel.put(key, reEncrypted);</span>
      }
&nbsp;
      const value = superjson.parse&lt;V&gt;(decryptedValue);
...
const getSessionStorage = (): Storage | undefined =&gt; {
  <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof globalThis !== 'undefined' &amp;&amp; 'sessionStorage' in globalThis) {
...
&nbsp;
  try {
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (storage) {
    ...
    console.debug(
      ...
      error instanceof Error ? error.message : <span class="branch-1 cbranch-no" title="branch not covered" >'Unknown error'</span>
    );
  }
&nbsp;
  console.warn(
...
&nbsp;
    async exportPrivateStates(options?: ExportPrivateStatesOptions): Promise&lt;PrivateStateExport&gt; {
      <span class="missing-if-branch" title="if path not taken" >I</span>if (contractAddress === null) {
<span class="cstat-no" title="statement not covered" >        throw new Error('Contract address not set. Call setContractAddress() before exporting private states.');</span>
      }
&nbsp;
...
&nbsp;
      ...
      for (const [scopedKey, value] of allStates.entries()) {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (scopedKey.startsWith(prefix)) {
...
&nbsp;
    async importPrivateStates(
      ...
      <span class="missing-if-branch" title="if path not taken" >I</span>if (contractAddress === null) {
<span class="cstat-no" title="statement not covered" >        throw new Error('Contract address not set. Call setContractAddress() before importing private states.');</span>
      }
&nbsp;
...
&nbsp;
        if (existingState !== null) {
          if (conflictStrategy === 'skip') {
            skipped++;
            continue;
          } else if (<span class="missing-if-branch" title="else path not taken" >E</span>conflictStrategy === 'overwrite') {
            overwritten++;
          }
        }
&nbsp;
