...
const makeAdaptedReader = (zkConfigProvider) => (compiledContract) => effect$1.Effect.gen(function* () {
    ...
    const getVerifierKey = (provableCircuitId) => effect$1.Effect.tryPromise({
        try: () => zkConfigProvider.getVerifierKey(provableCircuitId).then((verifierKey) => effect$1.Option.some(effect.Contract.VerifierKey(verifierKey))),
        catch: (err) => effect.ZKConfigurationReadError.make(compiledContract.tag, provableCircuitId, 'verifier-key', err)
    });
    return {
        getVerifierKey,
        getVerifierKeys: (provableCircuitIds) => effect$1.Effect.forEach(provableCircuitIds, (provableCircuitId) => getVerifierKey(provableCircuitId).pipe(effect$1.Effect.map((verifierKey) => [provableCircuitId, verifierKey])), { concurrency: 'unbounded', discard: false })
    };
});
 ...
 */
const createProofProvider = (provingProvider, costModel = ledgerV8.CostModel.initialCostModel()) => ({
...
Object.defineProperty(exports, "Transaction", {
    enumerable: true,
    get: function () { return ledgerV8.Transaction; }
});
...
exports.MAX_EXPORT_SIGNING_KEYS = MAX_EXPORT_SIGNING_KEYS;
