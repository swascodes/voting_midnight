...
const makeAdaptedReader = (zkConfigProvider) => (compiledContract) => Effect.gen(function* () {
    ...
    const getVerifierKey = (provableCircuitId) => Effect.tryPromise({
        try: () => zkConfigProvider.getVerifierKey(provableCircuitId).then((verifierKey) => Option.some(Contract.VerifierKey(verifierKey))),
        catch: (err) => ZKConfigurationReadError.make(compiledContract.tag, provableCircuitId, 'verifier-key', err)
    });
    return {
        getVerifierKey,
        getVerifierKeys: (provableCircuitIds) => Effect.forEach(provableCircuitIds, (provableCircuitId) => getVerifierKey(provableCircuitId).pipe(Effect.map((verifierKey) => [provableCircuitId, verifierKey])), { concurrency: 'unbounded', discard: false })
    };
});
 ...
 */
const createProofProvider = (provingProvider, costModel = CostModel.initialCostModel()) => ({
